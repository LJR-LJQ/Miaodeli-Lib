...
function queryDir(dirPathAbs, scb, fcb) {
	// 【变量】
	var index;
	var stat, items, item, pathAbs;
	var dirList, fileList;


	// 【过程】
	try {
		stat = fs.statSync(dirPathAbs);
	} catch(err) {
		fcb(); // 权限问题
		return;
	}
	if(!fs.existsSync(dirPathAbs) || !stat.isDirectory()) {
		fcb(); // 目录不存在，该路径不是有意义的目录
		return;
	}
	dirList = [];
	fileList = [];
	items = fs.readdirSync(dirPathAbs);
	for(index = 0; index < items.length; index++) {
		item = items[index];
		pathAbs = dirPathAbs + '/' + item;
		try {
			stat = fs.statSync(pathAbs);
			if(stat.isDirectory()) {
				dirList.push(item);
			} else {
				fileList.push(item);
...
// 【测试】
queryDir('c:/', function(dirList, fileList){
	console.log(dirList);
