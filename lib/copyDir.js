...
function copyDir(srcDirPathAbs, targetDirPathAbs, scb, fcb) {
	...
	// 【函数】
	function doCopyDir(srcDirPathAbs, targetDirPathAbs, scb, fcb){
		// 【参数】
		var files;
		// 【过程】
		fs.mkdirSync(targetDirPathAbs);
		var files = fs.readdirSync(srcDirPathAbs);
		doCopyFile(files, srcDirPathAbs, targetDirPathAbs, fcb, scb);
		return;
	}

	function doCopyFile(files, srcDirPathAbs, targetDirPathAbs, fcb, scb){
		// 【参数】
		...
		try {
			...
			if(fs.statSync(srcFilePathAbs).isDirectory()) {
				doCopyDir(srcFilePathAbs, targetFilePathAbs, function(){}, function(){});
				doCopyFile(files, srcDirPathAbs, targetDirPathAbs, fcb, scb);
			} else {
...
// 【测试】
copyDir('c:/kankan', 'd:/kankan', function(){}, function(){});